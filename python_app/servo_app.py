...
class ServoApp(ctk.CTkFrame):
    def __init__(self, master=None, return_callback=None, initial_tab=None):
        ...
        self.load_config()
        self.build_ui()
        if hasattr(self, 'initial_tab') and self.initial_tab:
            try:
    ...
    def build_ui(self):
        ...
        btn_volver = ctk.CTkButton(header_frame, text="Volver al Menú", fg_color="#da3633", hover_color="#f85149", font=ctk.CTkFont(size=14, weight="bold"), command=self.on_closing, width=120)
    ...
    def on_closing(self):
        self.running = False
        if getattr(self, "after_ids", None):
