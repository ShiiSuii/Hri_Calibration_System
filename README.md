...
- **Flujo Lógico**: El cableado define el orden de cascada.
...
6. 🔴 **Cámara HRI / Metric Logging (NUEVO)**: Nodo de infraestructura investigativa implementado para permitir a este entorno recopilar Big Data real orientada a la Interacción Social de Humanos y Robots. Al ejecutarse una orden de *Iniciar Tracking*, interrumpe la animación y exhibe de imprevisto una ventana Pop-up forzando al operador al ingreso de datos demográficos del participante a sentarse (`Nombre, Edad, Carrera`). Posteriormente reanuda toda la animación e invoca una hebra secundaria en la que `OpenCV` toma control de la WebCam y genera localmente y de forma rítmica (aprox. 10 FPS) mediciones documentadas en un `.csv`. (*Nota:* Posee un esqueleto abierto inyectable diseñado explícitamente a que al insertarle algún pipeline de *Computer Vision* como las redes de *DeepFace*, pueda adjuntar variables predictivas de *Emoción* o *Nivel de Atención* hacia el robot sin entorpecer la secuencia motora principal).
